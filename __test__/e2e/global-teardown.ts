...
import { deleteSeededEligibility } from "./fixtures/seeded-eligibility"
import { deleteSeededBanks } from "./fixtures/seeded-banks"
import { deleteSeededCategories } from "./fixtures/seeded-categories"
import { deleteSeededParticipantGroups } from "./fixtures/seeded-groups"
 ...
 *
 * Order matters: grants first (they cascade with schedules but pin groups
 * with FK RESTRICT), then schedules, packages, questions before banks
 * (question -> bank is RESTRICT), and groups before the media cleanups.
 * Media ledger rows survive question deletion (FK SET NULL), so they are
 * removed here, along with every object in the dedicated test bucket.
 */
export default async function globalTeardown() {
  ...
  await deleteSeededParticipantGroups()
